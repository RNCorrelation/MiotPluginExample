...
import {
  Button, Dimensions, findNodeHandle, FlatList, Image, PixelRatio, ScrollView, StyleSheet, Text, TextInput, TouchableHighlight, View
} from 'react-native';
...
export default class LHFileTestPage extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    FileEvent.fileDownloadProgress.addListener((data) => {
      const downloaded = data.downloadBytes;
      const all = data.totalBytes;
      const progress = downloaded / all * 100;
      const visProgress = progress < 100;
  ...
  _screenShot() {
    const imageName = 'screen_' + new Date().getTime() + '.png';
  ...
  _longScreenShot() {
    const node = findNodeHandle(this.refs.myScrollView);
    const imageName = 'screen_' + new Date().getTime() + '.png';
    ...
    return (
      <View>
        <TouchableHighlight
          ...
          onPress={() => { return this._readFile(item); }}
        >
          <Text>{item}</Text>
        </TouchableHighlight>
  ...
  _uploadFDSFile() {
    ...
    Host.file.generateObjNameAndUrlForFDSUpload(did, suffix).then((res) => {
      if (res.hasOwnProperty(suffix) && res[suffix]) {
        ...
        const { obj_name } = obj;
        const name = obj_name.substring(obj_name.length - 22);
        const content = 'AC';
        const { time } = obj;
  ...
  _downLoadFile() {
    ...
    const path = 'http://cdn.cnbj0.fds.api.mi-img.com/miio_fw/8229041d5532e4c8389cce0469299ef3_upd_inshow.watch.w1.bin?GalaxyAccessKeyId=5721718224520&Expires=1550972034000&Signature=yCSX06T+IKQ0RHkAznK+H8PRtlc=';
        ...
        >
          <View style={styles.row}>
            ...
            <TextInput
              ...
              style={{
                flex: 1, marginLeft: 10, borderBottomColor: '#aaa', borderBottomWidth: 1
              }}
              placeholder="输入文件名"
          ...
          <View style={styles.row}>
            ...
            <Button
              ...
              onPress={this.state.encoded ? () => { return this._decode(); } : () => { return this._encode(); }}
            />
          </View>
          <TextInput
          ...
          <View style={styles.row}>
            <View style={{ flex: 1 }}>
              <Button
                title="上传FDS文件"
                onPress={() => { return this._uploadFDSFile(); }}
              />
            </View>
            <View style={{ flex: 1 }}>
              <Button
                title="获取FDS文件"
                onPress={() => { return this._fetchFDSFile(); }}
              />
            </View>
          </View>
          <View style={styles.row}>
            <View style={{ flex: 1 }}>
              <Button
                title="下载文件"
                onPress={() => { return this._downLoadFile(); }}
              />
            </View>
            <View style={{ flex: 1 }}>
              <Button
                title="解压文件"
                onPress={() => { return this._unZipFile(); }}
              />
            </View>
            <View style={{ flex: 1 }}>
              <Button
                title="读文件列表"
                onPress={() => { return this._readFileListWithFolder(); }}
              />
            </View>
          </View>
          ...
          <View style={styles.row}>
            <View style={{ flex: 1 }}>
              <Button
                title="写文件"
                onPress={() => { return this._writeFile(); }}
              />
            </View>
            <View style={{ flex: 1 }}>
              <Button
                title="写文件(Base64)"
                onPress={() => { return this._writeFileThroughBase64(); }}
              />
            </View>
          </View>
          <View style={styles.row}>
            <View style={{ flex: 1 }}>
              <Button
                title="添加内容"
                onPress={() => { return this._appendFile(); }}
              />
            </View>
            <View style={{ flex: 1 }}>
              <Button
                title="添加内容(Base64)"
                onPress={() => { return this._appendFileThroughBase64(); }}
              />
            </View>
          </View>
          <View style={styles.row}>
            <View style={{ flex: 1 }}>
              <Button
                title="读文件(Base64)"
                onPress={() => { return this._readFileToBase64(); }}
              />
            </View>
            <View style={{ flex: 1 }}>
              <Button
                title="读文件(HexString)"
                onPress={() => { return this._readFileToHexString(); }}
              />
            </View>
          </View>
          <View style={styles.row}>
            <View style={{ flex: 1 }}>
              <Button
                ...
                onPress={() => { return this._getLengthOfBase64Data(); }}
              />
            </View>
            <View style={{ flex: 1 }}>
              <Button
                title="获取子data(half)"
                onPress={() => { return this._getSubBase64Data(); }}
              />
            </View>
          </View>
          <View style={styles.row}>
            <View style={{ flex: 1 }}>
              <Button
                title="截图当前页面"
                onPress={() => { return this._screenShot(); }}
              />
            </View>
            <View style={{ flex: 1 }}>
              <Button
                title="长截屏"
                onPress={() => { return this._longScreenShot(); }}
              />
            </View>
          </View>
          <View style={styles.row}>
            <View style={{ flex: 1 }}>
              <Button
                title="截图并保存到相册"
                onPress={() => { return this._screenShotAndSaveToPhotosAlbum(); }}
              />
            </View>
          </View>
          <View style={[styles.row, { justifyContent: 'center' }]}>
            <View style={{ flex: 1 }}>
              <Button
                title="删除当前显示的文件"
                onPress={() => { return this._deleteFile(); }}
              />
            </View>
            <View style={{ flex: 1 }}>
              <Button
                title="判断文件是否存在"
                onPress={() => { return this._isFileExist(); }}
              />
            </View>
          </View>
          ...
          <View style={{ flex: 1, flexDirection: 'row', margin: 5 }}>
            <View style={{ flex: 1, padding: 5 }}>
              ...
              <FlatList
                ...
                renderItem={({ item }) => { return this._renderFileList(item.name); }}
              />
            </View>
